...
export type Theme = "synnical" | "blossom" | "blood"

type BrowserState = {
  searchEngineId: string
  homepage: string
  useProxy: boolean
  useUltraviolet: boolean
  bookmarks: Bookmark[]
    ...
    (set, get) => ({
      ...
      addBookmark: (b) =>
        set((s) => {
          ...
          return { bookmarks: [...s.bookmarks, { ...b, id: crypto.randomUUID(), createdAt: Date.now() }] }
        }),
      removeBookmark: (id) => set((s) => ({ bookmarks: s.bookmarks.filter((x) => x.id !== id) })),
      isBookmarked: (url) => get().bookmarks.some((x) => x.url === url),
      recordVisit: (url, title) =>
        set((s) => ({
          history: [{ id: crypto.randomUUID(), url, title, visitedAt: Date.now() }, ...s.history.filter((h) => h.url !== url)].slice(0, 200),
        })),
